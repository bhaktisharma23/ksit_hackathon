...
import { getResultVideoUrl } from "../services/videoService";
...
export default function VideoAnalysis() {
  ...
  useEffect(() => {
    if (status !== "completed" || !video?.id) return;

    let cancelled = false;
    getResultVideoUrl(video.id).then((url) => {
      if (cancelled) {
    ...
    <PageContainer title="Video Analysis">
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="col-span-2">
          <VideoPlayer
            videoUrl={previewUrl}
            detections={isCompleted ? detections : []}
            seekTo={seekTo}
